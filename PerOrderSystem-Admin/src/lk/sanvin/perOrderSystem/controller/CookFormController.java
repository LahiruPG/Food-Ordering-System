 ...
 * @author LahiruPG
 */
public class CookFormController implements Initializable {

    ObservableList<CookDTO> List = FXCollections.observableArrayList();
    ...
    @FXML
    private TableColumn<CookDTO, String> clmName;
    @FXML
    private TableColumn<CookDTO, String> clmGender;
    @FXML
    private TableColumn<CookDTO, String> clmNIC;
    @FXML
    private TableColumn<CookDTO, String> clmDob;
    @FXML
    private TableColumn<CookDTO, String> clmContactNo;
    @FXML
    private TableColumn<CookDTO, String> clmPassword;
    @FXML
    private TableColumn<CookDTO, String> clmAddress;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            // TODO
            service = (CookService) ProxyHandler.getInstance().getService(ServiceFactory.ServiceTypes.COOK);
        } catch (Exception ex) {
                        ...
                        if (txtDob.getText().matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}$")) {
                            if (txtNic.getText().matches("^([A-Z a-z0-9.]){1,95}")) {
                                try {
                                    ...
                                    clearTextFields();
                                    loadTable();
                                    new Notifi("Added Sucess", "Sucessfully added new Cook");
                                } catch (Exception ex) {
                        ...
                        if (txtDob.getText().matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}$")) {
                            if (txtNic.getText().matches("^([A-Z a-z0-9.]){1,95}")) {
                                try {
                                    ...
                                    clearTextFields();
                                    loadTable();
                                    new Notifi("Update Sucess", "Sucessfully update a Cook");
                                } catch (Exception ex) {
                                    Logger.getLogger(TPOFormController.class.getName()).log(Level.SEVERE, null, ex);
                                }
                            } else {
 ...
 try {
            ...
            service.remove(dto);
            loadTable();
            new Notifi("Delete Sucess", "Sucessfully delete a Cook");
        } catch (Exception ex) {
            Logger.getLogger(TPOFormController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
