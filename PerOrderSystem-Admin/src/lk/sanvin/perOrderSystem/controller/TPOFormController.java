 ...
 * @author LahiruPG
 */
public class TPOFormController implements Initializable {

    ObservableList<TPODTO> tpoList = FXCollections.observableArrayList();
    @FXML
    ...
    @FXML
    private TableColumn<TPODTO, String> clmName;
    @FXML
    private TableColumn<TPODTO, String> clmGender;
    @FXML
    private TableColumn<TPODTO, String> clmNIC;
    @FXML
    private TableColumn<TPODTO, String> clmDob;
    @FXML
    private TableColumn<TPODTO, String> clmAddress;
    @FXML
    private TableColumn<TPODTO, String> clmContactNo;
    @FXML
    private TableColumn<TPODTO, String> clmPassword;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            // TODO
            service = (TPOService) ProxyHandler.getInstance().getService(ServiceFactory.ServiceTypes.TPO);
        } catch (Exception ex) {
                        ...
                        if (txtDob.getText().matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}$")) {
                            if (txtNic.getText().matches("^([A-Z a-z0-9.]){1,95}")) {
                                try {
                                    ...
                                    clearTextFields();
                                    loadTable();
                                    new Notifi("Update Sucess", "Sucessfully update a Telephone Operator");
                                } catch (Exception ex) {
                                    Logger.getLogger(TPOFormController.class.getName()).log(Level.SEVERE, null, ex);
                                }
                            } else {
    ...
    private void btnSDeleteAction(ActionEvent event) {
        try {
            ...
            service.remove(dto);
            loadTable();
            new Notifi("Delete Sucess", "Sucessfully delete a Telephone Operator");
        } catch (Exception ex) {
            Logger.getLogger(TPOFormController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
                        ...
                        if (txtDob.getText().matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}$")) {
                            if (txtNic.getText().matches("^([A-Z a-z0-9.]){1,95}")) {
                                try {
                                    ...
                                    clearTextFields();
                                    loadTable();
                                    new Notifi("Added Sucess", "Sucessfully added new Telephone Telephone Operator");
                                } catch (Exception ex) {
                                    Logger.getLogger(TPOFormController.class.getName()).log(Level.SEVERE, null, ex);
                                }
                            } else {
    ...
    private void loadTable() {
        try {
            ...
            List<TPODTO> asd = service.getAll();
            List<TPODTO> all = asd;
            for (TPODTO tpo : all) {
                tpoList.add(tpo);
            }
    ...
    private void tblGetItem() {
        tblTpo.setOnMousePressed(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
                String gen = "";

                id = tblTpo.getSelectionModel().getSelectedItem().getId();
                txtName.setText(tblTpo.getSelectionModel().getSelectedItem().getName());
                gen = tblTpo.getSelectionModel().getSelectedItem().getGender();
                txtAddress.setText(tblTpo.getSelectionModel().getSelectedItem().getAddress());
                txtContact.setText(tblTpo.getSelectionModel().getSelectedItem().getContact());
                txtDob.setText(tblTpo.getSelectionModel().getSelectedItem().getDob());
                txtNic.setText(tblTpo.getSelectionModel().getSelectedItem().getNic());
                txtPassword.setText(tblTpo.getSelectionModel().getSelectedItem().getPassword());
