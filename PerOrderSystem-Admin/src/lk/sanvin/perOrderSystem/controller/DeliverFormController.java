...
public class DeliverFormController implements Initializable {
    ...
    ObservableList<DeliverDTO> List = FXCollections.observableArrayList();
    ...
    @FXML
    private TableView<DeliverDTO> tblCook;
    @FXML
    private TableColumn<DeliverDTO, String> clmName;
    @FXML
    private TableColumn<DeliverDTO, String> clmGender;
    @FXML
    private TableColumn<DeliverDTO, String> clmNIC;
    @FXML
    private TableColumn<DeliverDTO, String> clmDob;
    @FXML
    private TableColumn<DeliverDTO, String> clmContactNo;
    @FXML
    private TableColumn<DeliverDTO, String> clmPassword;
    @FXML
    private TableColumn<DeliverDTO, String> clmAddress;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            // TODO
            service = (DeliverService) ProxyHandler.getInstance().getService(ServiceFactory.ServiceTypes.DELIVER);
        } catch (Exception ex) {
            Logger.getLogger(DeliverFormController.class.getName()).log(Level.SEVERE, null, ex);
        }
        initCol();
       loadTable();
       tblGetItem();
                        ...
                        if (txtDob.getText().matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}$")) {
                            if (txtNic.getText().matches("^([A-Z a-z0-9.]){1,95}")) {
                                try {
                                    ...
                                    clearTextFields();
                                    loadTable();
                                    new Notifi("Added Sucess", "Sucessfully added new Deliver");
                                } catch (Exception ex) {
                        ...
                        if (txtDob.getText().matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}$")) {
                            if (txtNic.getText().matches("^([A-Z a-z0-9.]){1,95}")) {
                                try {
                                    ...
                                    clearTextFields();
                                    loadTable();
                                    new Notifi("Update Sucess", "Sucessfully update a Deliver");
                                } catch (Exception ex) {
                                    Logger.getLogger(TPOFormController.class.getName()).log(Level.SEVERE, null, ex);
                                }
                            } else {
 ...
 try {
            ...
            service.remove(dto);
            loadTable();
            new Notifi("Delete Sucess", "Sucessfully delete a Deliver");
        } catch (Exception ex) {
            Logger.getLogger(TPOFormController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void loadTable() {
        try {
            List.clear();
           // List<CookDTO> all = service.getAll();
            List<DeliverDTO> all = service.getAll();
            for (DeliverDTO deliver : all) {
                List.add(deliver);
            }
