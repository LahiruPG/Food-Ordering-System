...
public class CashierFormController implements Initializable {
    ...
    ObservableList<CashierDTO> List = FXCollections.observableArrayList();
    ...
    @FXML
    private TableColumn<CashierDTO, String> clmName;
    @FXML
    private TableColumn<CashierDTO, String> clmGender;
    @FXML
    private TableColumn<CashierDTO, String> clmNIC;
    @FXML
    private TableColumn<CashierDTO, String> clmDob;
    @FXML
    private TableColumn<CashierDTO, String> clmContactNo;
    @FXML
    private TableColumn<CashierDTO, String> clmPassword;
    @FXML
    private TableColumn<CashierDTO, String> clmAddress;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            // TODO
            service = (CashierService) ProxyHandler.getInstance().getService(ServiceFactory.ServiceTypes.CASHIER);
        } catch (Exception ex) {
                        ...
                        if (txtDob.getText().matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}$")) {
                            if (txtNic.getText().matches("^([A-Z a-z0-9.]){1,95}")) {
                                try {
                                    ...
                                    clearTextFields();
                                    loadTable();
                                    new Notifi("Added Sucess", "Sucessfully added new Cashier");
                                } catch (Exception ex) {
                        ...
                        if (txtDob.getText().matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}$")) {
                            if (txtNic.getText().matches("^([A-Z a-z0-9.]){1,95}")) {
                                try {
                                    ...
                                    clearTextFields();
                                    loadTable();
                                    new Notifi("Update Sucess", "Sucessfully update a Cashier");
                                } catch (Exception ex) {
                                    Logger.getLogger(TPOFormController.class.getName()).log(Level.SEVERE, null, ex);
                                }
                            } else {
 ...
 try {
            ...
            service.remove(dto);
            loadTable();
            new Notifi("Delete Sucess", "Sucessfully delete a Cashier");
        } catch (Exception ex) {
            Logger.getLogger(TPOFormController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void loadTable() {
        try {
            List.clear();
            List<CashierDTO> all = service.getAll();
            for (CashierDTO tpo : all) {
                List.add(tpo);
            }
            tblCashier.getItems().setAll(List);
        } catch (Exception ex) {
            Logger.getLogger(TPOFormController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void tblGetItem() {
        tblCashier.setOnMousePressed(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
                String gen = "";

                id = tblCashier.getSelectionModel().getSelectedItem().getId();
                txtName.setText(tblCashier.getSelectionModel().getSelectedItem().getName());
                gen = tblCashier.getSelectionModel().getSelectedItem().getGender();
                txtAddress.setText(tblCashier.getSelectionModel().getSelectedItem().getAddress());
                txtContact.setText(tblCashier.getSelectionModel().getSelectedItem().getContact());
                txtDob.setText(tblCashier.getSelectionModel().getSelectedItem().getDob());
                txtNic.setText(tblCashier.getSelectionModel().getSelectedItem().getNic());
                txtPassword.setText(tblCashier.getSelectionModel().getSelectedItem().getPassword());
