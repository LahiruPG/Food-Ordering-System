 ...
 * @author LahiruPG
 */
public class TPORepositoryImpl extends SuperRepositoryImpl<TPO, String> implements TPORepository {

    private Session s;

    public TPORepositoryImpl() {
    }
    ...
    public List<TPO> finbyNicAndPassword(String nic, String password) {
        String query = "from TPO o where o.nic='"+ nic +"' and o.password='"+ password +"'";
    ...
    public List<TPO> finbyStatus(String status) {
        String query = "from TPO o where o.status='"+status+"'";
        List list = s.createQuery(query).list();
        return list;
    }
